...
/**
 * @brief Inicializa o IMU.
 *
 * Esta função configura o I2C e inicializa o sensor IMU com o endereço fornecido.
 *
 * @param devAddr Endereço do dispositivo IMU.
 * @param sda_pin Pino SDA do I2C.
 * @param scl_pin Pino SCL do I2C.
 * @return ESP_OK em caso de sucesso, ou ESP_ERR_NOT_FOUND se houver erro na configuração.
 */
esp_err_t imu_init(uint8_t devAddr, gpio_num_t sda_pin, gpio_num_t scl_pin)
{
    ...
    i2c_master_read(cmd, &tmp, 1, I2C_MASTER_LAST_NACK);
    i2c_master_stop(cmd);

    result_cmd_begin_write = i2c_master_cmd_begin(I2C_MASTER_NUM, cmd, pdMS_TO_TICKS(1000));
    switch (result_cmd_begin_write)
    {
    case ESP_OK:
        break;
    ...
    i2c_cmd_link_delete(cmd);
    // Fim do despertar do sensor

    return ESP_OK;
}

/**
 * @brief Obtém os dados de aceleração do sensor IMU.
 *
 * Esta função lê os dados de aceleração do sensor e os armazena na estrutura fornecida.
 *
 * @param data Ponteiro para a estrutura `AccelerationData` onde os dados de aceleração serão armazenados.
 * @return ESP_OK em caso de sucesso, ou ESP_FAIL se o ponteiro `data` for NULL.
 */
...
/**
 * @brief Obtém os dados do giroscópio do sensor IMU.
 *
 * Esta função lê os dados do giroscópio do sensor e os armazena na estrutura fornecida.
 *
 * @param data Ponteiro para a estrutura `GyroscopeData` onde os dados do giroscópio serão armazenados.
 * @return ESP_OK em caso de sucesso, ou ESP_FAIL se o ponteiro `data` for NULL.
 */
...
/**
 * @brief Desinstala o driver I2C do IMU.
 *
 * Esta função desinstala o driver I2C utilizado para comunicação com o sensor.
 *
 * @return ESP_OK em caso de sucesso, ou ESP_FAIL se houver erro ao desinstalar o driver.
 */
